...
// Note: Do NOT set runtime = 'edge'. @opennextjs/cloudflare uses the Node.js runtime.
...
export async function GET(request: NextRequest) {
  ...
  if (Math.abs(targetTime - now) < ONE_HOUR) {
    mode = 'current';
    url = `https://api.openweathermap.org/data/2.5/air_pollution?lat=${lat}&lon=${lon}&appid=${OPENWEATHER_API_KEY}`;
  } else if (targetTime > now && targetTime < now + FIVE_DAYS) {
    mode = 'forecast';
    url = `https://api.openweathermap.org/data/2.5/air_pollution/forecast?lat=${lat}&lon=${lon}&appid=${OPENWEATHER_API_KEY}`;
  } else {
    mode = 'history';
    const start = targetTime - ONE_HOUR;
    const end = targetTime + ONE_HOUR;
    url = `https://api.openweathermap.org/data/2.5/air_pollution/history?lat=${lat}&lon=${lon}&start=${start}&end=${end}&appid=${OPENWEATHER_API_KEY}`;
  }
  ...
  try {
    ...
    if (mode === 'forecast' || mode === 'history') {
      if (data.list && data.list.length > 0) {
        const closest = data.list.reduce(
          (prev: any, curr: any) =>
