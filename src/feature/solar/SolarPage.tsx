...
const SolarPage: React.FC = () => {
	...
	useEffect(() => {
		shadowCoverageRef.current = shadowCoverage;
	}, [shadowCoverage]);
	useEffect(() => {
		tariffRef.current = tariff;
	}, [tariff]);
	useEffect(() => {
		peakSunHoursRef.current = peakSunHours;
	}, [peakSunHours]);
	...
	const highlightBuilding = useCallback(
		(feature: any, monthlySavings: number) => {
			...
			if (
				...
				if (Cesium.defined(cartesian)) {
					...
					if (pickedObject instanceof Cesium.Cesium3DTileFeature) {
						const height =
							pickedObject.getProperty("cesium#estimatedHeight") || 15;
						const levels =
						...
						const dimensions: BuildingDimensions = estimateBuildingDimensions(
							height,
							levels,
							elementId ? String(elementId) : undefined,
						);
			...
			<div className="absolute top-5 left-5 z-10 w-80 p-6 bg-slate-900/40 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl text-white">
				<div className="space-y-6">
					...
					<div className="flex items-center justify-between p-3 bg-white/5 rounded-xl border border-white/10">
						...
						>
							<div
								className={`absolute top-1 left-1 w-4 h-4 bg-white rounded-full transition-transform ${showShadows ? "translate-x-6" : "translate-x-0"}`}
							/>
						</button>
					</div>
			...
			<div className="absolute top-5 right-5 z-10 flex gap-4">
				<div className="px-4 py-2 bg-slate-900/60 backdrop-blur-md rounded-xl border border-white/10 text-center">
					...
					<p className="text-lg font-bold text-yellow-400">
						5.8 <span className="text-[10px] text-white/60">kWh/m²</span>
					</p>
				</div>
				<div className="px-4 py-2 bg-slate-900/60 backdrop-blur-md rounded-xl border border-white/10 text-center">
					<p className="text-[10px] uppercase tracking-tighter text-white/40">
					...
					<p className="text-lg font-bold text-green-400">
						₹8.5 <span className="text-[10px] text-white/60">avg</span>
					</p>
				</div>
			</div>
				...
				<div className="p-4 space-y-3">
					...
					<div className="p-3 bg-yellow-500/10 border border-yellow-500/30 rounded-xl">
						...
						<p className="text-xl font-bold text-yellow-400">
							₹
							{Math.round(
								(1 - shadowCoverage) * tariff * peakSunHours * 30 * 4,
							).toLocaleString("en-IN")}
						</p>
						<p className="text-[10px] text-white/40 mt-1">
							Based on avg 4kW rooftop system
						</p>
					</div>
